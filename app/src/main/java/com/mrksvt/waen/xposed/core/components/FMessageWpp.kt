package com.mrksvt.waen.xposed.core.components
        ...
        get() {
            try {
                ...
                for (field in mediaClass.declaredFields) {
                    ...
                    val mediaObject = ReflectionUtils.getObjectField(field, fmessage) ?: continue
                    val fileGetter = mediaObject.javaClass.declaredMethods.firstOrNull {
                        it.returnType == File::class.java && it.parameterCount == 0
                    }
                    if (fileGetter != null) {
                        fileGetter.isAccessible = true
                        val f = fileGetter.invoke(mediaObject) as? File
                        if (f != null) return f
                    }
                    for (sf in mediaObject.javaClass.declaredFields) {
                        if (sf.type != String::class.java) continue
                        sf.isAccessible = true
                        val v = sf.get(mediaObject) as? String ?: continue
                        if (v.isBlank()) continue
                        val path = if (v.startsWith("file://")) android.net.Uri.parse(v).path else v
